...
"""
    ...
    if Y.shape[0]:
        ...
        for index, x in np.ndenumerate(Y):
            cross_entropy_array.append(-1.0 * np.log(A[int(x), index[1]]+epsilon))
...
def initialize_multilayer_weights(net_dims):
    ...
    #v=initialize_velocity(parameters,net_dims)
...
def update_parameters(parameters, gradients,v,s,t,learning_rate=0.1, beta1=0.9,beta2=0.999,epsilon=1e-8 , decay_rate=0.01):
    alpha = learning_rate
    L = len(parameters)//2
    v_c = {}
    s_c = {}
    ...
    for l in range(L):
        ...
        v_c["dW" + str(l + 1)] = v["dW" + str(l + 1)] / (1-np.power(beta1,t))
        v_c["db" + str(l + 1)] = v["db" + str(l + 1)] / (1-np.power(beta1,t))
        ...
        s_c["dW" + str(l + 1)] = s["dW" + str(l + 1)] / (1-np.power(beta2,t))
        s_c["db" + str(l + 1)] = s["db" + str(l + 1)] /( 1-np.power(beta2,t))
        parameters["W" + str(l + 1)] = parameters["W" + str(l + 1)] - learning_rate*v_c["dW" + str(l + 1)]/np.sqrt(s_c["dW" + str(l + 1)]+epsilon)
        parameters["b" + str(l + 1)] = parameters["b" + str(l + 1)] - learning_rate*v_c["db" + str(l + 1)]/np.sqrt(s_c["db" + str(l + 1)]+epsilon)
...
def multi_layer_network(X, Y, valid_x, valid_y,parameters,v,s,t,net_dims,num_iterations=500, learning_rate=0.2, decay_rate=0.01,beta=0.9):
    ...
    beta1=0.9
    beta2=0.999
    parameters,alpha,v,s = update_parameters(parameters,gradients,v,s,t,learning_rate, beta1,beta2,decay_rate)
    t=t+1
    ...
    return loss, parameters, v_loss,v,s,t
...
def main():
    ...
    #mini_batches_train=get_mini_batches(train_data,train_label,500)
    #mini_batches_val=get_mini_batches(validation_data,validation_label,100)
    mini_batches_train=[]
    mini_batches_val=[]
    for j in range(10):
    ...
    for i in range(len(mini_batches_train)):
        loss, parameters, v_loss,v,s,t = multi_layer_network(mini_batches_train[i][0],mini_batches_train[i][1],mini_batches_val[i][0],mini_batches_val[i][1],parameters,v,s,t,net_dims, \
                num_iterations=num_iterations,learning_rate=learning_rate,decay_rate=0.01,beta=beta)


        #print(v["dW1"])

        if i%10 ==0:
            costs_adam.append(loss)
            valid_costs_adam.append(v_loss)
    min_train_loss=min(costs_adam)
    print("min_train_loss",min_train_loss)
    with open('output_adam.txt', 'w') as f:
        for item in valid_costs_adam:
            f.write("%s\n" % item)
