...
def update_parameters(parameters, gradients,v,beta,learning_rate, decay_rate=0.0):
    '''
    ...
    return parameters,v, alpha

def multi_layer_network(X, Y, valid_x, valid_y,parameters,v,net_dims,num_iterations=500, learning_rate=0.2, decay_rate=0.01,beta=0.9):
    ...
    parameters,v, alpha = update_parameters(parameters,gradients,v,beta,learning_rate,decay_rate)
    ...
    return loss, parameters, v_loss,v
...
def main():
    ...
    costs=[]
    valid_costs=[]
    parameters,v= initialize_multilayer_weights(net_dims)
    for i in range(len(mini_batches_train)):
        loss, parameters, v_loss,v = multi_layer_network(mini_batches_train[i][0],mini_batches_train[i][1],mini_batches_val[i][0],mini_batches_val[i][1],parameters,v,net_dims, \
    ...
    with open('output_momentum.txt', 'w') as f:
